...
async function migrate() {
  ...
  // Use service role key for migrations, fallback to anon key
  ...
  if (error) {
    ...
    console.log('\n⚠️  Supabase free tier may not support custom SQL execution via RPC.')
    console.log('   Please run the SQL in src/database/migrate.sql manually via Supabase SQL Editor.\n')
    process.exit(1)
  }
